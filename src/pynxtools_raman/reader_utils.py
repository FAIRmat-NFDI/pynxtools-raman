...
class RamanMapper(ABC):
    ...
    def parse_file(self, file, **kwargs):
        """
        Parse the file using the Witec TXT parser.
...
def re_map_keys(dictionary: Dict[str, Any], key_map: Dict[str, str]):
    ...
    to the common metadata names. used in NXraman.

    Parameters
    ----------
    dictionary : Dict[str, Any]
        Dictionary with Raman metadata.
...
def re_map_values(dictionary: Dict[str, Any], map_functions: Dict[str, Any]):
    ...
    to the enumerations used in NXraman.
...
def construct_data_key(spectrum: Dict[str, Any]) -> str:
    """
    Construct a key for the 'data' field of the raman_dict.
