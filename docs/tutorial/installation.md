...
- how to install `pynxtools-raman`
- how to install `pynxtools-raman` together with NOMAD
...
### How to install `pynxtools-raman` with NOMAD

To use `pynxtools-raman` with NOMAD, simply install it in the same environment as the `nomad-lab` package. NOMAD will recognize `pynxtools-raman` as a plugin automatically. In addition, NOMAD will install a schema for NeXus application definitions.

## Start using `pynxtools-raman`

That's it! You can now use `pynxtools-raman`!
