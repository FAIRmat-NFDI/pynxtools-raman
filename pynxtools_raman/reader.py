...
class RamanReader(BaseReader):
    ...
    def read(
        ...
        if False:
            ...
            if header["Data"]["spectrum_unit_x"] == "nm":
                spectrum_unit = "nm"
            else:
            ...
            template[f"/ENTRY[entry]/DATA[data]/AXISNAME[{spectrum_type}]"] = {
                "link": f"/entry/DATA[data]/{spectrum_type}_spectrum"
            }
            template[
                f"/ENTRY[entry]/DATA[data]/NAME_spectrum[{spectrum_type}_spectrum]/@units"
            ] = spectrum_unit
            template[
                f"/ENTRY[entry]/DATA[data]/NAME_spectrum[{spectrum_type}_spectrum]/@long_name"
            ...
            for dindx in range(0, len(labels.keys())):
                for index, key in enumerate(data_list[dindx]):
                    template[f"/ENTRY[entry]/DATA[data]/DATA[{key}]"] = {
                        "link": "/entry/DATA[data]/spectrum_data_y",
                    ...
                    template[f"/ENTRY[entry]/DATA[data]/DATA[{key}]/@units"] = "degree"
                    if dindx == 0 and index == 0:
                        template[
                            f"/ENTRY[entry]/DATA[data]/DATA[{key}]/@long_name"
                        ] = f"{plot_name} (degree)"
                    template[f"/ENTRY[entry]/DATA[data]/DATA[{key}_errors]"] = {
                        "link": "/entry/DATA[data]/data_error",
                        "shape": np.index_exp[index, dindx, :],
                    }
                    # MK:: Carola, Ron, Flo, Tamas, Sandor refactor the following line
                    template[f"/ENTRY[entry]/DATA[data]/DATA[{key}_errors]/@units"] = (
                        "degree"
                    )

            # Define default plot showing cts vs n,:
            template["/@default"] = "entry"
            template["/ENTRY[entry]/@default"] = "DATA[data]"
            template["/ENTRY[entry]/DATA[data]/@signal"] = f"spectrum_data_y"
            template["/ENTRY[entry]/DATA[data]/@axes"] = f"{spectrum_type}" + "_spectrum_y"

            print("TEMPLATE:",template)

            # if len(data_list[0]) > 1:
            template["/ENTRY[entry]/DATA[data]/@auxiliary_signals"] = data_list[0][1:]
            for index in range(1, len(data_list)):
                template["/ENTRY[entry]/DATA[data]/@auxiliary_signals"] += data_list[
                    index
                ]

            template["/ENTRY[entry]/definition"] = "NXraman"
            template["/ENTRY[entry]/definition/@url"] = (
                "https://github.com/FAIRmat-NFDI/nexus_definitions/"
                f"blob/{get_nexus_version_hash()}/contributed_definitions/NXraman.nxdl.xml"
            )
