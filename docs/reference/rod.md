...
Reads `.rod` files — CIF-formatted records from the [Raman Open Database (ROD)](https://solsa.crystallography.net/rod/){:target="_blank" rel="noopener"}. Unlike WITec data, no separate ELN file is needed: a `.rod` file carries both the spectrum and its metadata. See [Learn > The WITec and ROD readers](../learn/readers.md#the-rod-sub-reader) for what the reader does with the data, and [How-to > Build a NOMAD upload batch from the Raman Open Database](../how-tos/build_a_rod_upload_batch.md) for downloading and converting many records at once.

## Supported format

| Format | Extension | Parser | Source |
| ------ | --------- | ------ | ------ |
| Raman Open Database record (CIF) | `.rod` | `RodParser` (via [`gemmi`](https://gemmi.readthedocs.io/){:target="_blank" rel="noopener"}) | `src/pynxtools_raman/rod/rod_reader.py` |
