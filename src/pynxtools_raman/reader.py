...
"""
A generic reader for loading Raman data
...
from pynxtools_raman.reader_utils import check_units

from pynxtools_raman.witec.txt.witec_reader import MapperWitec
...
CONVERT_DICT = {
    ...
    "source_raman": "source_TYPE[source_raman]",
    "beam_raman": "beam_TYPE[beam_raman]",
...
def concatenate_values(value1, value2):
    ...
    in raman_data_dict. Dicts are merged and every other object is
    appended to a list.
...
class RamanReader(MultiFormatReader):
    ...
    __file_err_msg__ = (
        "Need an Raman data file with one of the following extensions: "
        f"{__prmt_file_ext__}"
    )

    __vndr_err_msg__ = (
        "Need an Raman data file from one of the following vendors: " f"{__vendors__}"
    )
    ...
    def handle_eln_file(self, file_path: str) -> Dict[str, Any]:
        ...
        for key, value in eln_data.copy().items():
            ...
            # REMARK RON: This is called for all keys, even if non of these is present? 
    ...
    def handle_data_file(self, file_path: str) -> Dict[str, Any]:
        def _check_for_vendors(file_path: str) -> str:
            ...
            vendor_dict = RamanReader.__prmt_vndr_cls[file_ext]
        ...
        def _check_for_vendors_txt(file_path: str) -> str:
            ...
            """
            vendor_dict = RamanReader.__prmt_vndr_cls[".txt"]
            ...
            for vendor in vendor_dict:
                ...
                if any(vendor_opt in contents for vendor_opt in vendor_options):
                    return vendor
                if contents[:6] == "[Info]":
                    # This is for picking the Witec reader is "witec"
                    # is not in the file
                    return vendor
            return "unknown"
        ...
        if file_ext in RamanReader.__prmt_file_ext__:
            vendor = _check_for_vendors(file_path)
            try:
                parser = RamanReader.__prmt_vndr_cls[file_ext][vendor]()

                parser.parse_file(file_path, **self.kwargs)
                self.config_file = RamanReader.reader_dir.joinpath(
                    "config", parser.config_file
                )
                data_dict = parser.data_dict

            except ValueError as val_err:
                raise ValueError(RamanReader.__vndr_err_msg__) from val_err
            except KeyError as key_err:
                raise KeyError(RamanReader.__vndr_err_msg__) from key_err
        else:
            raise ValueError(RamanReader.__file_err_msg__)
    ...
    def _combine_datasets(self) -> None:
        ...
        combines the different data sets from the Raman files and ensures
        ...
        common_entries, dict_indices = check_for_same_entries(self.raman_data_dicts)

        if common_entries:
            for entry, indices in zip(common_entries, dict_indices):
                dicts_with_common_entries = [self.raman_data_dicts[i] for i in indices]
        ...
        for data_dict in self.raman_data_dicts:
            # If there are multiple input data files of the same type,
            # make sure that existing keys are not overwritten.
            existing = [
                (key, self.raman_data[key], data_dict[key])
                for key in set(self.raman_data).intersection(data_dict)
            ]

            self.raman_data = {**self.raman_data, **data_dict}
            for key, value1, value2 in existing:
                self.raman_data[key] = concatenate_values(value1, value2)
    ...
    def _get_detector_names(self) -> List[str]:
        ...
        try:
            for entry, entry_values in self.raman_data["data"].items():
    ...
    def get_metadata(
        ...
        """
        Get metadata from the ELN or Raman data dictionaries.
        ...
        ----------
        metadata_dict : Dict[str, Any]
            One of ELN or Raman data dictionaries .
        path : str
